...
from .auto_config import _try_satellite6_configuration, _try_satellite5_configuration
...
def _is_client_registered(config):
    ...
    if config.analyze_container:
        return (
            "Client running in container/image mode. Bypassing registration check",
            False,
        )
...
def try_register(config):
    ...
    if config.display_name is None and config.group is None:
        ...
        logger.info(
            "Successfully registered host %s as %s in group %s",
            hostname,
            display_name,
            group,
        )
...
def get_branch_info(config):
    ...
    if config.offline or config.analyze_container:
        return branch_info
...
def collect(config):
    ...
    if config.analyze_file:
        ...
        target = {
            ...
            "name": os.path.splitext(os.path.basename(config.analyze_file))[0],
    ...
    # mountpoints
        ...
        target = {
            "type": "mountpoint",
            "name": os.path.splitext(os.path.basename(config.analyze_mountpoint))[0],
    ...
    try:
        ...
        if (config.from_file or config.from_stdin) and (
            "uploader.json" not in stdin_config or "sig" not in stdin_config
        ):
            raise ValueError
        if (
            (config.from_file or config.from_stdin)
    ...
    try:
        ...
        # analyze mountpoints
            ...
            logging_name = (
                "Filesystem " + target["name"] + " at location " + target["location"]
            )
        ...
        archive = InsightsArchive(
            compressor=config.compressor, target_name=target["name"]
        )
...
def upload(config, tar_file, collection_duration=None):
    ...
    for tries in range(config.retries):
        ...
        if upload.status_code in (200, 201):
            ...
            logger.error(
                "Upload attempt %d of %d failed! Status Code: %s",
                tries + 1,
                config.retries,
                upload.status_code,
            )
            if tries + 1 != config.retries:
                ...
                logger.error(
                    "Please see %s for additional information", config.logging_file
                )
    return api_response
