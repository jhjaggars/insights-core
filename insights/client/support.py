...
def registration_check(pconn):
    ...
    if type(api_reg_status) is bool:
        ...
        api_record = (
            "Insights API says this machine was unregistered at " + api_reg_status
        )
...
class InsightsSupport(object):
    ...
    def collect_support_info(self):
        ...
        tar_file = os.path.join(
            log_archive_dir,
            "insights-client-logs-" + time.strftime("%Y%m%d%H%M%S") + ".tar.gz",
        )
    ...
    def _support_diag_dump(self):
        ...
        if self.config.proxy:
            obfuscated_proxy = re.sub(
                r"(.*)(:)(.*)(@.*)", r"\1\2********\4", self.config.proxy
            )
        else:
