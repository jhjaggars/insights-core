...
class InsightsClient(object):
    ...
    def fetch(self, force=False):
        ...
        # run fetch for egg
        updated = self._fetch(
            self.config.egg_path, constants.core_etag_file, fetch_results["core"], force
        )
    ...
    def verify(self, egg_path, gpg_key=constants.pub_gpg_path):
        ...
        if egg_path and not os.path.isfile(egg_path):
            the_message = "Provided egg path %s does not exist, cannot verify." % (
                egg_path
            )
        ...
        if self.config.gpg and gpg_key and not os.path.isfile(gpg_key):
            the_message = (
                "Running in GPG mode but cannot find "
        ...
        if not self.config.gpg:
            return {"gpg": True, "stderr": None, "stdout": None, "rc": 0}
    ...
    def install(self, new_egg, new_egg_gpg_sig):
        ...
        try:
            if not os.path.isdir(constants.insights_core_lib_dir):
                logger.debug(
                    "Creating directory %s for the Core."
        ...
        try:
            logger.debug(
                "Copying %s to %s." % (new_egg, constants.insights_core_newest)
            )
    ...
    def update_rules(self):
        ...
        if self.config.offline or not self.config.auto_update:
            logger.debug(
                "Bypassing rule update due to config "
    ...
    def get_registration_information(self):
        ...
        return {
            "machine-id": client.get_machine_id(),
    ...
    def get_last_upload_results(self):
        ...
        if os.path.isfile(constants.last_upload_results_file):
            logger.debug(
                "Last upload file %s found, reading results.",
