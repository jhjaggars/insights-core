...
DEFAULT_OPTS = {
    ...
    "retries": {
        ...
        "opt": ["--retry"],
        "help": (
            "Number of times to retry uploading. %s seconds between tries"
    ...
    "to_stdout": {
        "default": False,
        "opt": ["--to-stdout"],
        "help": "print archive to stdout; " "sets --quiet and --no-upload",
        "action": "store_true",
    },
    "unregister": {
        "default": False,
...
class InsightsConfig(object):
    ...
    def __str__(self):
        ...
        for key in dir(self):
            if not (
                key.startswith("_")
                or key in self._init_attrs
                or key in ["password", "proxy"]
            ):
    ...
    def _update_dict(self, dict_):
        ...
        if unknown_opts:
            raise ValueError("Unknown options: " + ",".join(list(unknown_opts)))
    ...
    def _imply_options(self):
        ...
        self.to_stdout = self.to_stdout or self.from_stdin or self.from_file
        self.to_json = (self.to_json or self.analyze_container) and not self.to_stdout
