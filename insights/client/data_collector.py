    ...
    def _parse_command_spec(self, spec, precmds):
        ...
        if "pre_command" in spec:
            ...
            try:
                ...
                logger.debug(
                    "Pre-command %s not found. Skipping %s...",
                    precmd_alias,
                    spec["command"],
                )
                return []
        ...
        if metadata_spec in conf:
            for spec in conf[metadata_spec]:
                if "file" in spec:
                    ...
                    if (
                        rm_conf
                        and "files" in rm_conf
                        and spec["file"] in rm_conf["files"]
                    ):
                        ...
                        for s in file_specs:
                            file_spec = InsightsFile(
                                s, exclude, self.mountpoint, self.target_name
                            )
                    ...
                    for g in glob_specs:
                        if (
                            rm_conf
                            and "files" in rm_conf
                            and g["file"] in rm_conf["files"]
                        ):
                            logger.warn("WARNING: Skipping file %s", g)
                            continue
                        else:
                            glob_spec = InsightsFile(
                                g, exclude, self.mountpoint, self.target_name
                            )
                            self.archive.add_to_archive(glob_spec)
                elif "command" in spec:
                    if (
                        rm_conf
                        and "commands" in rm_conf
                        and spec["command"] in rm_conf["commands"]
                    ):
    ...
    def run_collection(self, conf, rm_conf, branch_info):
        ...
        if self.config.run_specific_specs is not None:
            ...
            for specific_spec in self.config.run_specific_specs.split(","):
                ...
                self.run_specific_specs(
                    specific_spec, conf, rm_conf, exclude, branch_info
                )
