...
class InsightsConstants(object):
    ...
    default_cmd_timeout = (
        600
    )  # default command execution to ten minutes, prevents long running commands that will hang
    ...
    core_gpg_sig_etag_file = os.path.join(
        default_conf_dir, ".insights-core-gpg-sig.etag"
    )
    ...
    insights_ansible_facts_file = os.path.join(
        insights_ansible_facts_dir, "insights.fact"
    )
    insights_ansible_machine_id_file = os.path.join(
        insights_ansible_facts_dir, "insights_machine_id.fact"
    )
    ...
    insights_core_last_stable_gpg_sig = os.path.join(
        insights_core_lib_dir, "last_stable.egg.asc"
    )
