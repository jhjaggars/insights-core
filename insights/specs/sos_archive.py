...
class SosSpecs(Specs):
    ...
    hostname = first_of(
        [
            simple_file("sos_commands/general/hostname_-f"),
            simple_file("sos_commands/general/hostname"),
        ]
    )
    httpd_conf_sos = glob_file(
        ["/conf/httpd/conf/httpd.conf", "/conf/httpd/conf.d/*.conf"]
    )
    ...
    puppet_ssl_cert_ca_pem = simple_file(
