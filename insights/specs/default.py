...
class DefaultSpecs(Specs):
    ...
    ceilometer_conf = first_file(
        [
            "/var/lib/config-data/ceilometer/etc/ceilometer/ceilometer.conf",
            "/etc/ceilometer/ceilometer.conf",
        ]
    )
    ceph_socket_files = listdir("/var/run/ceph/ceph-*.*.asok", context=HostContext)
    ceph_config_show = foreach_execute(
        ceph_socket_files, "/usr/bin/ceph daemon %s config show"
    )
    ...
    ceph_osd_ec_profile_get = foreach_execute(
        ceph_osd_ec_profile_ls,
        "/usr/bin/ceph osd erasure-code-profile get %s -f json-pretty",
    )
    ...
    certificates_enddate = simple_command(
        "/usr/bin/find /etc/origin/node /etc/origin/master /etc/pki -type f -exec /usr/bin/openssl x509 -noout -enddate -in '{}' \; -exec echo 'FileName= {}' \;"
    )
    ...
    cobbler_modules_conf = first_file(
        ["/etc/cobbler/modules.conf", "/conf/cobbler/modules.conf"]
    )
    ...
    docker_image_inspect = foreach_execute(
        docker_image_ids, "/usr/bin/docker inspect %s"
    )
    docker_container_inspect = foreach_execute(
        docker_container_ids, "/usr/bin/docker inspect %s"
    )
    ...
    fc_match = simple_command(
        "/bin/fc-match -sv 'sans:regular:roman' family fontformat"
    )
    ...
    foreman_rake_db_migrate_status = simple_command(
        "/usr/sbin/foreman-rake db:migrate:status"
    )
    fstab = simple_file("/etc/fstab")
    galera_cnf = first_file(
        [
            "/var/lib/config-data/mysql/etc/my.cnf.d/galera.cnf",
    ...
    gnocchi_conf = first_file(
        [
            "/var/lib/config-data/gnocchi/etc/gnocchi/gnocchi.conf",
            "/etc/gnocchi/gnocchi.conf",
        ]
    )
    gnocchi_metricd_log = first_file(
        [
            "/var/log/containers/gnocchi/gnocchi-metricd.log",
            "/var/log/gnocchi/metricd.log",
        ]
    )
    ...
    heat_api_log = first_file(
        ["/var/log/containers/heat/heat-api.log", "/var/log/heat/heat-api.log"]
    )
    heat_conf = first_file(
        ["/var/lib/config-data/heat/etc/heat/heat.conf", "/etc/heat/heat.conf"]
    )
    heat_crontab = simple_command("/usr/bin/crontab -l -u heat")
    heat_crontab_container = simple_command(
        "docker exec heat_api_cron /usr/bin/crontab -l -u heat"
    )
    heat_engine_log = first_file(
        ["/var/log/containers/heat/heat-engine.log", "/var/log/heat/heat-engine.log"]
    )
    ...
    keystone_crontab_container = simple_command(
        "docker exec keystone_cron /usr/bin/crontab -l -u keystone"
    )
    keystone_log = first_file(
        ["/var/log/containers/keystone/keystone.log", "/var/log/keystone/keystone.log"]
    )
    ...
    libkeyutils_objdumps = simple_command(
        '/usr/bin/find -L /lib /lib64 -name libkeyutils.so.1 -exec objdump -x "{}" \;'
    )
    libvirtd_log = simple_file("/var/log/libvirt/libvirtd.log")
    limits_conf = glob_file(
    ...
    lvs = (
        None
    )  # simple_command('/sbin/lvs -a -o +lv_tags,devices --config="global{locking_type=0}"')
    lvs_noheadings = simple_command(
        "/sbin/lvs --nameprefixes --noheadings --separator='|' -a -o lv_name,lv_size,lv_attr,mirror_log,vg_name,devices,region_size,data_percent,metadata_percent --config=\"global{locking_type=0}\""
    )
    lvs_noheadings_all = simple_command(
        "/sbin/lvs --nameprefixes --noheadings --separator='|' -a -o lv_name,lv_size,lv_attr,mirror_log,vg_name,devices,region_size,data_percent,metadata_percent --config='global{locking_type=0} devices{filter=[\"a|.*|\"]}'"
    )
    ...
    metadata_json = simple_file(
        "metadata.json", context=ClusterArchiveContext, kind=RawFileProvider
    )
    mlx4_port = simple_command(
    ...
    mysql_log = glob_file(
        ["/var/log/mysql.log", "/var/opt/rh/rh-mysql*/log/mysql/mysqld.log"]
    )
    ...
    networkmanager_dispatcher_d = glob_file(
    ...
    nova_api_log = first_file(
        ["/var/log/containers/nova/nova-api.log", "/var/log/nova/nova-api.log"]
    )
    nova_compute_log = first_file(
        ["/var/log/containers/nova/nova-compute.log", "/var/log/nova/nova-compute.log"]
    )
    nova_conf = first_file(
        ["/var/lib/config-data/nova/etc/nova/nova.conf", "/etc/nova/nova.conf"]
    )
    nova_crontab = simple_command("/usr/bin/crontab -l -u nova")
    nova_crontab_container = simple_command(
        "docker exec nova_api_cron /usr/bin/crontab -l -u nova"
    )
    ...
    numeric_user_group_name = simple_command(
        "/bin/grep -c '^[[:digit:]]' /etc/passwd /etc/group"
    )
    oc_get_bc = simple_command(
        "/usr/bin/oc get bc -o yaml --all-namespaces", context=OpenShiftContext
    )
    oc_get_build = simple_command(
        "/usr/bin/oc get build -o yaml --all-namespaces", context=OpenShiftContext
    )
    oc_get_dc = simple_command(
        "/usr/bin/oc get dc -o yaml --all-namespaces", context=OpenShiftContext
    )
    oc_get_egressnetworkpolicy = simple_command(
        "/usr/bin/oc get egressnetworkpolicy -o yaml --all-namespaces",
        context=OpenShiftContext,
    )
    oc_get_endpoints = simple_command(
        "/usr/bin/oc get endpoints -o yaml --all-namespaces", context=OpenShiftContext
    )
    oc_get_event = simple_command(
        "/usr/bin/oc get event -o yaml --all-namespaces", context=OpenShiftContext
    )
    oc_get_node = simple_command(
        "/usr/bin/oc get nodes -o yaml", context=OpenShiftContext
    )
    oc_get_pod = simple_command(
        "/usr/bin/oc get pod -o yaml --all-namespaces", context=OpenShiftContext
    )
    oc_get_project = simple_command(
        "/usr/bin/oc get project -o yaml --all-namespaces", context=OpenShiftContext
    )
    oc_get_pv = simple_command(
        "/usr/bin/oc get pv -o yaml --all-namespaces", context=OpenShiftContext
    )
    oc_get_pvc = simple_command(
        "/usr/bin/oc get pvc -o yaml --all-namespaces", context=OpenShiftContext
    )
    oc_get_rc = simple_command(
        "/usr/bin/oc get rc -o yaml --all-namespaces", context=OpenShiftContext
    )
    oc_get_role = simple_command(
        "/usr/bin/oc get role -o yaml --all-namespaces", context=OpenShiftContext
    )
    oc_get_rolebinding = simple_command(
        "/usr/bin/oc get rolebinding -o yaml --all-namespaces", context=OpenShiftContext
    )
    oc_get_route = simple_command(
        "/usr/bin/oc get route -o yaml --all-namespaces", context=OpenShiftContext
    )
    oc_get_service = simple_command(
        "/usr/bin/oc get service -o yaml --all-namespaces", context=OpenShiftContext
    )
    odbc_ini = simple_file("/etc/odbc.ini")
    odbcinst_ini = simple_file("/etc/odbcinst.ini")
    crt = simple_command(
        "/usr/bin/find /etc/origin/node /etc/origin/master -type f -path '*.crt'"
    )
    openshift_certificates = foreach_execute(
        crt, "/usr/bin/openssl x509 -noout -enddate -in %s"
    )
    openvswitch_other_config = simple_command(
        "/usr/bin/ovs-vsctl -t 5 get Open_vSwitch . other_config"
    )
    ...
    def package_and_java(broker):
        ...
        for p in ps:
            ...
            which = (
                ctx.shell_out("which {0}".format(cmd))
                if "java" in os.path.basename(cmd)
                else None
            )
            resolved = (
                ctx.shell_out("readlink -e {0}".format(which[0])) if which else None
            )
    ...
    md5chk_files = simple_command(
        "/bin/ls -H /usr/lib*/{libfreeblpriv3.so,libsoftokn3.so} /etc/pki/product*/69.pem /etc/fonts/fonts.conf /dev/null 2>/dev/null"
    )
    prelink_orig_md5 = None
    prev_uploader_log = simple_file(
        "var/log/redhat-access-insights/redhat-access-insights.log.1"
    )
    ...
    pvs = simple_command(
        '/sbin/pvs -a -v -o +pv_mda_free,pv_mda_size,pv_mda_count,pv_mda_used_count,pe_count --config="global{locking_type=0}"'
    )
    pvs_noheadings = simple_command(
        "/sbin/pvs --nameprefixes --noheadings --separator='|' -a -o pv_all,vg_name --config=\"global{locking_type=0}\""
    )
    pvs_noheadings_all = simple_command(
        "/sbin/pvs --nameprefixes --noheadings --separator='|' -a -o pv_all,vg_name --config='global{locking_type=0} devices{filter=[\"a|.*|\"]}'"
    )
    qemu_conf = simple_file("/etc/libvirt/qemu.conf")
    qemu_xml = glob_file(r"/etc/libvirt/qemu/*.xml")
    qpid_stat_q = simple_command(
        "/usr/bin/qpid-stat -q --ssl-certificate=/etc/pki/katello/qpid_client_striped.crt -b amqps://localhost:5671"
    )
    qpid_stat_u = simple_command(
        "/usr/bin/qpid-stat -u --ssl-certificate=/etc/pki/katello/qpid_client_striped.crt -b amqps://localhost:5671"
    )
    qpidd_conf = simple_file("/etc/qpid/qpidd.conf")
    rabbitmq_logs = glob_file(
        "/var/log/rabbitmq/rabbit@*.log", ignore=".*rabbit@.*(?<!-sasl).log$"
    )
    rabbitmq_policies = simple_command("/usr/sbin/rabbitmqctl list_policies")
    rabbitmq_queues = simple_command(
    ...
    rhn_entitlement_cert_xml = first_of(
        [
            glob_file("/etc/sysconfig/rhn/rhn-entitlement-cert.xml*"),
    ...
    rhn_server_xmlrpc_log = first_file(
        ["/var/log/rhn/rhn_server_xmlrpc.log", "/rhn-logs/rhn/rhn_server_xmlrpc.log"]
    )
    ...
    rhn_taskomatic_daemon_log = first_file(
        [
    ...
    rpm_V_packages = simple_command(
        "/usr/bin/rpm -V coreutils procps procps-ng shadow-utils passwd sudo"
    )
    ...
    subscription_manager_list_consumed = simple_command(
        "/usr/bin/subscription-manager list --consumed"
    )
    subscription_manager_list_installed = simple_command(
        "/usr/bin/subscription-manager list --installed"
    )
    subscription_manager_repos_list_enabled = simple_command(
        "/usr/bin/subscription-manager repos --list-enabled"
    )
    swift_object_expirer_conf = first_file(
        [
            "/var/lib/config-data/swift/etc/swift/object-expirer.conf",
            "/etc/swift/object-expirer.conf",
        ]
    )
    swift_proxy_server_conf = first_file(
        [
            "/var/lib/config-data/swift/etc/swift/proxy-server.conf",
    ...
    sysconfig_memcached = first_file(
        [
            "/var/lib/config-data/memcached/etc/sysconfig/memcached",
    ...
    sysctl_conf_initramfs = simple_command(
        "/bin/lsinitrd /boot/initramfs-*kdump.img -f /etc/sysctl.conf /etc/sysctl.d/*.conf"
    )
    systemctl_cinder_volume = simple_command(
    ...
    systemd_openshift_node = simple_file(
    ...
    teamdctl_state_dump = foreach_execute(
        ethernet_interfaces, "/usr/bin/teamdctl %s state dump"
    )
    ...
    tomcat_server_xml = first_of(
        [
            foreach_collect(tomcat_base, "%s/conf/server.xml"),
    ...
    tomcat_vdc_targeted = foreach_execute(
        tomcat_home_base, "/bin/grep -R -s 'VirtualDirContext' --include '*.xml' %s"
    )
    ...
    vgs = (
        None
    )  # simple_command('/sbin/vgs -v -o +vg_mda_count,vg_mda_free,vg_mda_size,vg_mda_used_count,vg_tags --config="global{locking_type=0}"')
    vgs_noheadings = simple_command(
        "/sbin/vgs --nameprefixes --noheadings --separator='|' -a -o vg_all --config=\"global{locking_type=0}\""
    )
    vgs_noheadings_all = simple_command(
        "/sbin/vgs --nameprefixes --noheadings --separator='|' -a -o vg_all --config='global{locking_type=0} devices{filter=[\"a|.*|\"]}'"
    )
    ...
    woopsie = simple_command(
        r"/usr/bin/find /var/crash /var/tmp -path '*.reports-*/whoopsie-report'"
    )
    ...
    host_installed_rpms = simple_command(
        "/usr/bin/rpm -qa --qf '%s'" % rpm_format, context=HostContext
    )
    ...
    def jboss_home(broker):
        ...
        for p in ps:
            if any(
                i in p for i in ["-D[Standalone]", "-D[Host Controller]", "-D[Server:"]
            ):
                found = findall(p)
                if found:
                    # Only get the path which is absolute
    ...
    jboss_domain_server_log = foreach_collect(
        jboss_domain_server_log_dir, "%s/server.log*"
    )
    ...
    jboss_standalone_main_config = foreach_collect(
        jboss_standalone_main_config_files, "%s"
    )
