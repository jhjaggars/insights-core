...
def test_all_modprobe():
    ...
    assert all_data["alias"] == {
        ...
        "megaraid_sas": ModProbeValue(
            value=["scsi_hostadapter"], source=MOD_COMPLETE_PATH
        ),
        "ata_piix": ModProbeValue(
            value=["scsi_hostadapter1"], source=MOD_COMPLETE_PATH
        ),
    }
    ...
    assert all_data.bad_lines[0] == ModProbeValue(
        value="alias", source=MOD_COMPLETE_PATH
    )
    assert all_data.bad_lines[1] == ModProbeValue(
        value="alias scsi_hostadapter2 ata_piix failed comment",
        source=MOD_COMPLETE_PATH,
    )
    assert all_data.bad_lines[2] == ModProbeValue(
        value="balclkist ieee80211", source=MOD_COMPLETE_PATH
    )
