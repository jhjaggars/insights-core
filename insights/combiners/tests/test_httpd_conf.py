...
def test_active_httpd_directory():
    ...
    assert result.get_section_list("Directory") == [
        (("Directory", "/var/www/example"), "httpd.conf", "/etc/httpd/conf/httpd.conf")
    ]
...
def test_active_httpd_nest_1():
    ...
    assert sorted(
        result.get_active_setting("Order", section=("FilesMatch", "php"))
    ) == sorted(
        [
            (
                "deny,allow",
    ...
    assert sorted(
        result.get_active_setting("RewriteEngine", section="IfModule")
    ) == sorted(
        [
            (
                "Off",
    ...
    assert (
        result.get_active_setting("Allow", ("FilesMatch", "test"))[0].value
        == "from all"
    )
    assert result.get_active_setting("Deny", section=("IfModule",)) == []
    assert (
        result.get_active_setting("MaxClients", section=("IfModule", "prefork")) == []
    )
    assert (
        result.get_active_setting("RewriteRule", section=("IfModule", "mod_rewrite.c"))[
            0
        ].line
        == "RewriteRule .* /index.php"
    )
...
def test_active_httpd_nest_2():
    httpd1 = HttpdConf(
        context_wrap(HTTPD_CONF_NEST_3, path="/etc/httpd/conf/httpd.conf")
    )
    httpd2 = HttpdConf(
        context_wrap(HTTPD_CONF_NEST_4, path="/etc/httpd/conf.d/00-z.conf")
    )
    result = HttpdConfAll([httpd1, httpd2])
...
def test_active_httpd():
    ...
    assert (
        result.get_active_setting("MaxClients", section=("IfModule", "prefork.c"))[
            0
        ].value
        == "512"
    )
    ...
    assert (
        result.get_active_setting("ThreadsPerChild", section=("IfModule", "prefork.c"))[
            0
        ].value
        == "16"
    )
    ...
    assert (
        result.get_active_setting("ServerLimit", section=("IfModule", "prefork.c"))[
            0
        ].value
        == "256"
    )
    assert (
        result.get_active_setting("JustForTest", section=("IfModule", "prefork.c"))[
            -1
        ].file_name
        == "00-z.conf"
    )
    assert (
        result.get_active_setting("JustForTest_NoSec").line
        == 'JustForTest_NoSec "/var/www/cgi"'
    )
...
def test_shadowing():
    httpd1 = HttpdConf(
        context_wrap(HTTPD_CONF_SHADOWTEST_1, path="/etc/httpd/conf/httpd.conf")
    )
    httpd2 = HttpdConf(
        context_wrap(HTTPD_CONF_SHADOWTEST_2, path="/etc/httpd/conf.d/00-z.conf")
    )
    httpd3 = HttpdConf(
        context_wrap(HTTPD_CONF_SHADOWTEST_3, path="/etc/httpd/conf.d/z-z.conf")
    )
    ...
    assert result.get_setting_list("Foo") == [
        ParsedData(
            "1A", "Foo 1A", None, None, "httpd.conf", "/etc/httpd/conf/httpd.conf"
        ),
        ParsedData(
            "1B", "Foo 1B", None, None, "httpd.conf", "/etc/httpd/conf/httpd.conf"
        ),
        ParsedData(
            "1C", "Foo 1C", None, None, "httpd.conf", "/etc/httpd/conf/httpd.conf"
        ),
        ParsedData(
            "2A", "Foo 2A", None, None, "00-z.conf", "/etc/httpd/conf.d/00-z.conf"
        ),
        ParsedData(
            "2B", "Foo 2B", None, None, "00-z.conf", "/etc/httpd/conf.d/00-z.conf"
        ),
        ParsedData(
            "2C", "Foo 2C", None, None, "00-z.conf", "/etc/httpd/conf.d/00-z.conf"
        ),
        ParsedData(
            "3A", "Foo 3A", None, None, "z-z.conf", "/etc/httpd/conf.d/z-z.conf"
        ),
        ParsedData(
            "3B", "Foo 3B", None, None, "z-z.conf", "/etc/httpd/conf.d/z-z.conf"
        ),
        ParsedData(
            "3C", "Foo 3C", None, None, "z-z.conf", "/etc/httpd/conf.d/z-z.conf"
        ),
    ]
    ...
    assert result.get_active_setting("Foo") == (
        "3C",
        "Foo 3C",
        None,
        None,
...
def test_httpd_splits():
    ...
    assert (
        result.get_active_setting("ServerRoot").line == 'ServerRoot "/home/skontar/www"'
    )
    assert result.get_active_setting("ServerRoot").file_name == "01-b.conf"
    assert (
        result.get_active_setting("ServerRoot").file_path
        == "/etc/httpd/conf.d/01-b.conf"
    )
    ...
    assert (
        result.get_active_setting("Listen").file_path == "/etc/httpd/conf.d/00-a.conf"
    )
