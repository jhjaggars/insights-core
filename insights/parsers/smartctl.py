...
class SMARTctl(CommandParser):
    ...
    _INFO_LINE_STR = r"(?P<key>\w+(?:\s\w+)*):\s+" + r"(?P<value>\S.*?)\s*$"
    _INFO_LINE_RE = re.compile(_INFO_LINE_STR)
    _VALUE_LINE_STR = r"(?P<key>\w[A-Za-z _.-]+):\s+" + r"\(\s*(?P<value>\S.*?)\)"
    _VALUE_LINE_RE = re.compile(_VALUE_LINE_STR)
    _ATTR_LINE_STR = (
        r"^\s*(?P<id>\d+)\s(?P<name>\w+)\s+"
    ...
    def __init__(self, context):
        ...
        if match:
            ...
            raise ParseException(
                "Cannot parse device name from path {p}".format(p=context.path)
            )
    ...
    def parse_content(self, content):
        ...
        def parse_values(line):
            ...
            if match:
                ...
                self.full_line = ""
            elif self.full_line.startswith(
        ...
        parse_for_state = [parse_information, parse_values, parse_attributes]
