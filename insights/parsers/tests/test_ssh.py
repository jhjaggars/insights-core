...
def test_sshd_config_complete():
    ...
    assert (
        config.get_line("ClientAliveInterval")
        == "ClientAliveInterval   # Implicit default"
    )
    assert (
        config.get_line("ClientAliveCountMax", "0")
        == "ClientAliveCountMax 0  # Implicit default"
    )
    ...
    assert (
        config.get_values("HostKey", default="no_key", join_with=" ")
