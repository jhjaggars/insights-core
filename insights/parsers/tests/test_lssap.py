...
def test_lssap_netweaver():
    ...
    data = sap.data
    assert keys_in(
        ["SID", "Nr", "Instance", "SAPLOCALHOST", "Version", "DIR_EXECUTABLE"], data[0]
    )
    assert data[0] == {
        "SID": "HA2",
        "Nr": "16",
...
def test_fail():
    ...
    assert (
        "Lssap: Unable to parse 1 line(s) of content: (['/usr/sap/hostctrl/lssap: command or file not found'])"
        in str(excinfo)
    )
    ...
    assert (
        "Lssap: Unable to parse 2 line(s) of content: (['HA2 | N16 | D16', 'HB2 | foo | bar'])"
        in str(excinfo)
    )

    with pytest.raises(ParseException) as excinfo:
        lssap.Lssap(context_wrap(Lssap_bad_instance_TEST))
    assert "Lssap: Invalid instance parsed in content: ['B', 'QR', 'DS']" in str(
        excinfo
    )
