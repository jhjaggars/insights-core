...
def test_pam_conf():
    ...
    assert (
        repr(pam_conf[0]) == "<PamConfEntry for vsftpd: auth required pam_securetty.so>"
    )
    assert (
        repr(pam_conf[4])
        == "<PamConfEntry for other: password include pam_cracklib.so retry=3 logging=verbose>"
    )
...
def test_bad_pamd_files():
    ...
    for entry in pamd_conf:
        ...
        assert entry._errors == [
            "Cannot parse line '{f}' as a valid pam.d entry".format(f=entry._full_line)
        ]
...
def test_pam_doc_examples():
    env = {
        ...
        "pamd_conf": PamDConf(
            context_wrap(PAMD_CONF_SSHD_DOCS, path="/etc/pam.d/sshd")
        ),
    }
