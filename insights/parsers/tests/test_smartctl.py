...
def test_standard_drive():
    ...
    assert (
        data.attributes["Airflow_Temperature_Cel"]["raw_value"] == "41 (Min/Max 21/41)"
    )
...
def test_bad_device():
    ...
    with pytest.raises(ParseException) as exc:
        assert (
            SMARTctl(context_wrap(STANDARD_DRIVE, path="sos_commands/ata/smartctl_-a"))
            is None
        )
...
def test_netapp_drive():
    ...
    assert data.information["Logical block size"] == "512 bytes"
    assert (
        data.information["Logical Unit id"]
        == "0x60a9800044312d364d5d4478753370620x5d447875337062000a980044312d364d"
    )
