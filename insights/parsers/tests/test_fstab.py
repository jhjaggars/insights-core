...
def test_fstab():
    ...
    assert results.search(fs_spec__startswith="LABEL=") == [
        l for l in results if l.fs_spec.startswith("LABEL")
    ]
    assert results.search(fs_mntops__contains="uid") == [
        l for l in results if "uid" in l.fs_mntops
    ]
    assert results.search(fs_vfstype="xfs", fs_mntops__contains="relatime") == [
        l for l in results if l.fs_vfstype == "xfs" and "relatime" in l.fs_mntops
    ]

    results = fstab.FSTab(context_wrap(content_fstab_without_mntopts))
    sitedata_mount_list = [
        result for result in results if result.fs_file == "/ceSiteData"
    ]
