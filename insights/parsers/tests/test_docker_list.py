...
def test_docker_list_images():
    ...
    assert (
        result.rows[0].get("IMAGE ID")
        == "412b684338a1178f0e5ad68a5fd00df01a10a18495959398b2cf92c2033d3d02"
    )
    ...
    assert (
        result.rows[1].get("IMAGE ID")
        == "34c167d900afb820ecab622a214ce3207af80ec755c0dcb6165b425087ddbc3a"
    )
    assert result.rows[2].get("REPOSITORY") == "<none>"
    assert result.rows[2].get("TAG") == "<none>"
    assert (
        result.rows[2].get("IMAGE ID")
        == "76e65756ff110ca5ea54ac02733fe04301b33a9190689eb524dd5aa18843996a"
    )
...
def test_docker_list_containers():
    ...
    assert (
        result.rows[0].get("CONTAINER ID")
    ...
    assert (
        result.rows[1].get("CONTAINER ID")
        == "95516ea08b565e37e2a4bca3333af40a240c368131b77276da8dec629b7fe102"
    )
    assert (
        result.rows[1].get("COMMAND")
        == "\"/bin/sh -c 'yum install -y vsftpd-2.2.2-6.el6'\""
    )
