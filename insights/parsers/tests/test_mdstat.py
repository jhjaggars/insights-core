...
MD_TEST_1 = (
    "md0 : active raid6 sdf1[0] sde1[1] sdd1[2](F) sdc1[3] sdb1[4](F) sda1[5] hdb1[6]"
)
MD_RESULT_1 = [
    {
        "device_name": "md0",
...
def test_mdstat_construction():
    def compare_mdstat_data(test_data, parser_obj):
        ...
        for testdata, objdata in zip(
            test_data["components"], parser_obj.data["components"]
        ):
    ...
    assert sorted(md_d0["devices"][0].keys()) == sorted(
        ["component_name", "role", "up"]
    )
