...
class TestGrub1ConfMissingBootFiles:
    def test_get_grub_kernel_initrd(self):
        expected_result = {
            "grub_kernels": [
                "vmlinuz-2.6.18-194.8.1.el5",
                "vmlinuz-2.6.18-194.17.1.el5",
            ],
            "grub_initrds": [
                "initrd-2.6.18-194.8.1.el5.img",
                "initrd-2.6.18-194.17.1.el5.img",
            ],
        }
        ...
        assert (
            expected_result == Grub1Config(context_wrap(GRUB_CONF_LINKS)).kernel_initrds
        )
        ...
        assert (
            expected_result == Grub1Config(context_wrap(GRUB_CONF_XEN)).kernel_initrds
        )
...
class TestGrub2ConfMissingBootFiles:
    def test_get_grub_kernel_initrd(self):
        ...
        assert (
            expected_result
            == Grub2Config((context_wrap(GRUB2_CFG_1))).kernel_initrds
            == expected_result
        )
        expected_result = {
            "grub_kernels": [
