...
def test_chrony_conf():
    ...
    assert (
        ntp_obj.get_last("noclientlog", default="yes") is None
    )  # present, so no default
...
def test_standard_ntp_conf():
    ...
    assert data == {
        ...
        "iburst": None,
        "server": ["127.127.1.0", "10.20.30.40", "192.168.1.111"],
        "fudge": ["127.127.1.0 stratum 10"],
        "peer": ["ntp1.example.com", "ntp2.example.com", "ntp3.example.com"],
    }

    # Test other attributes
    assert hasattr(ntp_obj, "servers")
    assert ntp_obj.servers == ["10.20.30.40", "127.127.1.0", "192.168.1.111"]
    assert hasattr(ntp_obj, "peers")
    assert ntp_obj.peers == ["ntp1.example.com", "ntp2.example.com", "ntp3.example.com"]
...
def test_ntp_get_tinker():
    ...
    assert ntp_obj.get_last("tinker", "panic") is None
    assert (
        ntp_obj.get_last("tinker", default="1") == "step 0.4"
    )  # No param: just find last line
    ...
    assert ntp_obj.get_last("tinker", "step") == "0.4"
    assert (
        ntp_obj.get_last("tinker", param="step", default="1") == "0.4"
    )  # Value from config
