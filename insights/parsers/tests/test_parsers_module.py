...
def test_split_kv_pairs():
    ...
    assert kv_pairs == {"keyword1": "value1", "keyword2": "value2a=True, value2b=100M"}
    ...
    kv_pairs = split_kv_pairs(
        SPLIT_TEST_1.splitlines(), use_partition=True, ordered=True
    )
    assert len(kv_pairs) == 3
    ...
    kv_pairs = split_kv_pairs(
        SPLIT_TEST_2.splitlines(), comment_char="@", split_on=":", use_partition=True
    )
    assert len(kv_pairs) == 3
    assert kv_pairs == {
        "keyword1": "value1",
...
def test_unsplit_lines():
    ...
    assert (
        lines[1] == "Line two part 1          line two part 2         line two part 3"
    )
    ...
    lines = list(
        unsplit_lines(SPLIT_LINES_3.splitlines(), cont_char=",", keep_cont_char=True)
    )
    assert len(lines) == 4
    assert lines[0] == ""
    assert (
...
def test_calc_offset():
    ...
    with pytest.raises(ValueError):
        calc_offset(OFFSET_CONTENT_1.splitlines(), target=["data "], invert_search=True)
    assert (
        calc_offset(
            OFFSET_CONTENT_1.splitlines(),
...
def test_parse_fixed_table():
    ...
    assert data[4] == {
        "Column1": "fooTrailing",
        "Column_2": "non-data li",
        "Column_3": "ne",
    }
    assert data[5] == {
        "Column1": "foo Another",
        "Column_2": "trailing no",
        "Column_3": "n-data line",
    }
...
def test_parse_delimited_table():
    ...
    # Header substitution
    tbl = parse_delimited_table(
        SUBSTITUTE_HEADERS_TEST.splitlines(),
        delim=",",
        strip=False,
    ...
    tbl = parse_delimited_table(
        POSTGRESQL_LOG.splitlines(), delim="|", trailing_ignore=["("]
    )
    assert isinstance(tbl, list)
    assert len(tbl) == 14
    assert isinstance(tbl[0], dict)
    ...
    expected = [
        {"THIS": "this", "IS": "is", "A": "some", "HEADER": "content"},
