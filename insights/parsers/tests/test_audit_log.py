...
add_filter("audit_log", ["CRYPTO_KEY_USER", "LOGIN"])
...
def test_audit_log():
    ...
    assert (
        line0.get("msg")
        == 'op=destroy kind=session fp=? direction=both spid=16865 suid=0 rport=59296 laddr=192.0.2.1 lport=22  exe="/usr/sbin/sshd" hostname=? addr=10.66.136.139 terminal=? res=success'
    )
    ...
    auditlog = AuditLog(
        context_wrap(AUDIT_LOG_TEMPALTE % AUDIT_LOG_WITH_JUST_HEAD_PART)
    )
    ...
    auditlog = AuditLog(
        context_wrap(AUDIT_LOG_TEMPALTE % AUDIT_LOG_WITHOUT_JUST_TAIL_PART)
    )
    info = auditlog.get("type=")
    assert len(info) == 4
    line2 = info[2]
