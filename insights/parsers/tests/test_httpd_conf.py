...
def test_get_httpd_conf_nest_1():
    ...
    assert (
        result[("VirtualHost", "192.0.2.1")][("IfModule", "mod_php4.c")][
            "php_admin_flag"
        ][-1].value
        == "safe_mode Off"
    )
    ...
    assert (
        result[("VirtualHost", "192.0.2.1")]["ServerName"][-1].value
        == "www.example.com"
    )
...
def test_get_httpd_conf_1():
    ...
    assert (
        result["LoadModule"][0].value == "auth_basic_module modules/mod_auth_basic.so"
    )
    assert (
        result["LoadModule"][-1].value
...
def test_main_config_splitting():
    ...
    assert result.file_path == HTTPD_CONF_PATH
    assert result.file_name == "httpd.conf"
    assert result["LogLevel"] == [
        ("warn", "LogLevel warn", None, None, result.file_name, result.file_path)
    ]
    assert result["EnableSendfile"] == [
        ("on", "EnableSendfile on", None, None, result.file_name, result.file_path)
    ]
